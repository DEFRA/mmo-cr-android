...
description: "Do thorough, risk-scoped internet research in the open and align findings to the DEFRA standards precedence (DEFRA > GDS > Google/Android > community) for the MMO Catch Recording Android app. Use for the single Research (§4.2) pass of the working framework — validating APIs, patterns, security and policy against Google/Android, DEFRA/GDS and framework (Kotlin/Jetpack Compose) guidance, and citing sources before a plan is approved or implemented. There is no separate validation-research round; the plan is checked against these same cited sources."
argument-hint: "e.g. 'validate the Android Keystore + WorkManager sync approach the planner flagged' or 'research Room vs SQLDelight for offline-first'"
...
- **The planner performs this single research pass** to validate those steps before returning the plan: the
  **Android Planner** for Complex work, or the **Android Developer** when it produces the lightweight inline
  plan for Standard work (or runs standalone). The parent agent only coordinates and checks the citations —
  it does not commission a second research round.

## When to use
- **Research (§4.2):** an unfamiliar API, framework, pattern, or policy point is genuinely uncertain.
- A DEFRA/GDS/Android requirement is ambiguous and could change the design.
...
Keystore**, **offline persistence & sync/conflict resolution**, **networking/TLS/network-security-config**,
**accessibility** (a legal requirement), **Google Play policy**, or a **version-sensitive API** (minSdk 26
minimum). A cosmetic or well-trodden step needs little or none.
...
4. **Google/Android** — Material Design 3, Android app architecture & quality guidance, Google Play policies
5. **Community best practice** — OWASP MASVS, widely-adopted Jetpack Compose patterns

> DEFRA beats GDS; GDS beats Google/Android/community. Any deviation from a DEFRA standard is a **governance
...
data-at-rest, error logging, WCAG 2.2 AA, minSdk 26 minimum), and what a good answer must let you decide.

### 2. Research in the open, current-first
- Search **authoritative, current** sources: Android Developer docs / Now in Android, DEFRA & GOV.UK
  standards and service manuals, OWASP MASVS, and the framework's own docs. Prefer primary sources over
  blog posts.
- **Confirm currency:** check the API/pattern is supported on the **minSdk 26 minimum** and is not
  deprecated. Note API-level availability (`@RequiresApi`) and any migration since.
...
- [ ] **Encrypt in transit** — HTTPS/TLS only, network security config blocks cleartext; no plain HTTP.
- [ ] **Protect data at rest** — Android Keystore for secrets, encrypted storage for sensitive data.
...
- [ ] **No secrets in code**; Gradle-version-catalog dependencies; minSdk 26 minimum honoured.
- [ ] **Currency** — API/pattern is current, non-deprecated, and available on minSdk 26+.
...
**revise** / **blocked**) so the plan can be finalised against it. When the **Android Planner** produced the
plan, send **revise/blocked** items back to it rather than fixing the plan yourself; when the **Android
...
- Instructions: [Security](../../instructions/security.instructions.md) · [Accessibility](../../instructions/accessibility.instructions.md) · [Kotlin/Compose](../../instructions/kotlin-compose.instructions.md)
